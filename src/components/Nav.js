...
class Nav extends Component {
  shouldComponentUpdate(newProps, newState) {
    // state가 변경되거나 새로운 props를 전달받을 떄 실행된다.
    // newProps와 newState는 각각이 실행될때 바뀐 값 ,
    ...
    console.log("바뀌기전의 값 =>>", this.props.data);

    // data값이 바뀌면 render가 호출되고 값이 바뀌지 않으면 호출되지 않게 한다.
